...
func negotiateMainlineTCPTransport(dtlsConn net.Conn) error {
	hello, err := sessionv3.BuildProbeHelloWithTransport(
	...
	if serverHello.GetVersion() != muxProtocolV3 {
		return fmt.Errorf("mainline TCP transport requires protocol v3, got v%d", serverHello.GetVersion())
	}
